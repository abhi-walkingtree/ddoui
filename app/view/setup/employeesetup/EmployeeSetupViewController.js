...
Ext.define('DDO.view.setup.employeesetup.EmployeeSetupViewController', {
    ...
    setEmployeeWorkStatus: function(workDetailsForm, record, me){
        ...
        if (!(empStatus == null)) {
            ...
            if (empStatus == "Probation") {
                me.doProbationProcess(workDetailsForm, confirmedDt, separatedDt, noticeDt);
            } else if (empStatus == "Confirmed") {
                me.doConfimedProcess(workDetailsForm, confirmedDt, separatedDt, noticeDt, record);
            } else if (empStatus == "Separated") {
                me.doSeparatedProcess(workDetailsForm, confirmedDt, separatedDt, noticeDt, record);
            } else if (empStatus == "Notice") {
                me.doNoticeProcess(workDetailsForm, confirmedDt, separatedDt, noticeDt, record);
            }
        }
    },
     ...
     */
    doProbationProcess: function(workDetailsForm, confirmedDt, separatedDt, noticeDt){
     ...
     * @param {record} - object, contains reference of selected row record from grid.
     */
    doConfimedProcess: function(workDetailsForm, confirmedDt, separatedDt, noticeDt, record){
     ...
     * @param {record} - object, contains reference of selected row record from grid.
     */
    doNoticeProcess: function(workDetailsForm, confirmedDt, separatedDt, noticeDt, record){
    ...
    */
    ...
    processSearchOperation: function(EmployeesetupGridStore, searchValue, allEmployeeBtnPressed){
        ...
        EmployeesetupGridStore.filterBy(function (record) {
            if (record.get('basic').firstname) {
                var firstName = record.get('basic').firstname.toLowerCase();
            }
            if (record.get('basic').lastname) {
