    ...
    onActivatingUserProfile:function(){
       ...
       empStore.load({
        callback: function(records,operation,success){
              if(success == true){
                var hrDetailsArray = records[0].data.managerHr;
                hrDetailsArray.forEach(value => {
                    if(logIn_Id == value.ddo_employee_id){
                        profileRefs.provitiontocnfm.show(); 
                    }
                })
                  records.forEach(value => {
                      basicDetails = value.data.basic;
                      if (id == basicDetails.ddo_employee_id){
                          reportingID = value.data.workdetails.reportingto;
                          if(reportingID == logIn_Id){
                              profileRefs.provitiontocnfm.show();
                          }else{
